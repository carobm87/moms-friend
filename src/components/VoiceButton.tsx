...
export function VoiceButton({ onTranscript, onInterim, disabled }: Props) {
  ...
  // Use refs for callbacks so the recognition instance never needs to be recreated
  const listeningRef = useRef(false);
  const finalTextRef = useRef("");
  const onTranscriptRef = useRef(onTranscript);
  const onInterimRef = useRef(onInterim);

  // Keep callback refs in sync without triggering recreation of the recognizer
  ...
  useEffect(() => {
    ...
    rec.onerror = (e: any) => {
      ...
      if (e.error === "not-allowed") {
        ...
        // iOS fires no-speech quickly — restart if still meant to be listening
    ...
    rec.onend = () => {
      // iOS Safari stops automatically after a short pause.
      // If the user hasn't tapped stop, restart immediately.
      if (listeningRef.current) {
        try {
          rec.start();
          return; // still recording — don't fire transcript yet
        } catch (e) {
          console.error("restart failed", e);
        }
      }

      // User tapped stop (or restart failed) — deliver the transcript
      setListening(false);
  ...
  }, []); // Empty deps — create the recognizer only once
